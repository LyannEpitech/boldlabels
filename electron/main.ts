...
function initDatabase() {
  ...
  const dbPath = path.join(userDataPath, 'boldlabels.db');
...
ipcMain.handle('db:createTemplate', (_, template: any) => {
  ...
  stmt.run(
    id, template.name, template.description || '', template.width, template.height,
    template.unit || 'mm', template.backgroundColor || '#FFFFFF',
    template.borderWidth || 0, template.borderColor || '#000000',
    template.borderRadius || 0, now, now
  );
  ...
  if (template.elements?.length > 0) {
    ...
    for (const el of template.elements) {
      elementStmt.run(
        crypto.randomUUID(), id, el.type, el.variableName, el.x, el.y,
        el.width, el.height, el.rotation || 0, JSON.stringify(el.properties), el.zIndex || 0
      );
    }
  }
...
ipcMain.handle('db:updateTemplate', (_, id: string, updates: any) => {
  const now = new Date().toISOString();
  ...
  stmt.run(
    updates.name, updates.description, updates.width, updates.height,
    updates.backgroundColor, updates.borderWidth, updates.borderColor,
    updates.borderRadius, now, id
  );
...
ipcMain.handle('db:getMappingsByTemplate', (_, templateId: string) => {
...
ipcMain.handle('db:createMapping', (_, mapping: any) => {
  const now = new Date().toISOString();
  const id = crypto.randomUUID();
  ...
  stmt.run(
    id, mapping.name, mapping.templateId,
    JSON.stringify(mapping.columnMappings),
    mapping.csvSample ? JSON.stringify(mapping.csvSample) : null,
    now, now
  );
...
ipcMain.handle('fs:savePDF', async (_, pdfData: ArrayBuffer, defaultFilename: string) => {
